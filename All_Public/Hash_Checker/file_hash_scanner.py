...
# IMPORTANT: Your Threat Intelligence API Key
...
def calculate_file_hashes(file_path):
    """
    Calculates MD5, SHA1, and SHA256 hashes of a given file.
    ...
    try:
        ...
        return {"error": f"Error calculating file hashes: {e}"}
...
def check_hash_reputation(hash_value):
    ...
    try:
        ...
        if response.status_code == 404:
            # Hash not found in the TI database
...
def generate_html_report_hash(file_name, file_hashes, hash_analysis_result):
    """
    Generates an HTML report for the file and hash analysis result, using existing CSS.
    """
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    queried_hash = hash_analysis_result.get('hash', 'N/A')
    report_title = f"XenoByte File & Hash Analysis Report: {file_name}"

    # --- CSS Styling (Copied from previous script) ---
    custom_css = """
    ...
    # --- JavaScript for theme toggling (Copied from previous script) ---
    theme_toggle_js = """
    <script>
    ...
    if hash_analysis_result.get("error"):
        ...
        html_sections.append(f"<h2>File Analysis Result</h2>")
        html_sections.append(f"<p><strong>Original File Name:</strong> {file_name}</p>")
        ...
        html_sections.append("<h3>Calculated File Hashes</h3>")
        html_sections.append("<ul>")
        html_sections.append(f"<li><strong>MD5:</strong> {file_hashes.get('md5', 'N/A')}</li>")
        html_sections.append(f"<li><strong>SHA1:</strong> {file_hashes.get('sha1', 'N/A')}</li>")
        html_sections.append(f"<li><strong>SHA256:</strong> {file_hashes.get('sha256', 'N/A')}</li>")
        html_sections.append(f"<li><strong>File Size:</strong> {file_hashes.get('file_size_bytes', 'N/A')} bytes</li>")
        html_sections.append("</ul>")

        # Additional File Information for the queried hash (from TI /general endpoint)
        html_sections.append("<h3>Threat Intelligence General File Information (Queried Hash)</h3>")
        html_sections.append("<ul>")
        html_sections.append(f"<li><strong>MD5 (TI):</strong> {hash_analysis_result.get('md5', 'N/A')}</li>")
        html_sections.append(f"<li><strong>SHA1 (TI):</strong> {hash_analysis_result.get('sha1', 'N/A')}</li>")
        html_sections.append(f"<li><strong>SHA256 (TI):</strong> {hash_analysis_result.get('sha256', 'N/A')}</li>")
        html_sections.append(f"<li><strong>File Size (TI):</strong> {hash_analysis_result.get('file_size', 'N/A')}</li>")
        html_sections.append(f"<li><strong>File Type (TI):</strong> {hash_analysis_result.get('file_type', 'N/A')}</li>")
        html_sections.append(f"<li><strong>Malware Family (TI, if detected):</strong> {hash_analysis_result.get('malware_family', 'N/A')}</li>")
        html_sections.append(f"<li><strong>First Submission (TI):</strong> {hash_analysis_result.get('first_submission_date', 'N/A')}</li>")
        html_sections.append(f"<li><strong>Last Analysis (TI):</strong> {hash_analysis_result.get('last_analysis_date', 'N/A')}</li>")
        html_sections.append("</ul>")
...
if __name__ == "__main__":
    print("--- XenoByte File Hash Scanner & Analyzer ---")
    print("This script calculates file hashes and checks its reputation using Threat Intelligence.")
    print("Ensure your Threat Intelligence API key is configured in the script.")
    print("-" * 30)

    file_path_input = input("Enter the path to the file you want to scan: ").strip()

    if not file_path_input:
        print("[!] File path cannot be empty. Exiting.")
        sys.exit(1)

    print(f"[*] Calculating hashes for file: {file_path_input}...")
    file_hashes_result = calculate_file_hashes(file_path_input)

    if "error" in file_hashes_result:
        print(f"ERROR: {file_hashes_result['error']}", file=sys.stderr)
        sys.exit(1)

    print(f"    [+] MD5: {file_hashes_result['md5']}")
    print(f"    [+] SHA1: {file_hashes_result['sha1']}")
    print(f"    [+] SHA256: {file_hashes_result['sha256']}")
    print(f"    [+] File Size: {file_hashes_result['file_size_bytes']} bytes")

    # Use SHA256 for reputation check as it's the strongest and most commonly used for TI
    sha256_to_check = file_hashes_result['sha256']
    
    analysis_result = check_hash_reputation(sha256_to_check)

    # Get the base file name for the report title
    file_name_only = os.path.basename(file_path_input)

    # Generate and save HTML report
    print("\n[+] Generating HTML report...")
    html_report_content = generate_html_report_hash(file_name_only, file_hashes_result, analysis_result)

    report_filename = f"XenoByte_File_Report_{file_name_only.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    try:
        ...
        print("\n[+] File analysis complete. Report opened in browser.")
    except IOError as e:
