...
# IMPORTANT: Ensure Wallet_ID_Monitor_Forensic.py is in the same directory as this API script
...
app = FastAPI(
    title="XenoByte Ethereum Wallet Forensic API",
    description="API for performing forensic analysis on Ethereum wallet addresses by calling the external forensic script.",
    version="1.0.0"
)
...
async def startup_event():
    ...
    Forensic analysis will be delegated to Wallet_ID_Monitor_Forensic.py.
    """
...
# --- API Endpoint ---
@app.get("/api/ethereum/forensic", response_model=Dict[str, Any], summary="Perform Ethereum Wallet Forensic Analysis")
async def perform_ethereum_forensic_analysis_endpoint(
    wallet_address: str = Query(..., description="The Ethereum wallet address for forensic analysis."),
    max_depth: int = Query(2, description="Maximum tracing depth (e.g., 1, 2, 3, or -1 for full depth). Default is 2."),
    api_key: str = Security(get_api_key)
):
    """
    Performs a forensic analysis on a specified Ethereum wallet address by executing
    the `Wallet_ID_Monitor_Forensic.py` script as a subprocess and returning its JSON output.
    The transaction flow graph SVG will be included as a base64 encoded string in the JSON response.

    **Parameters:**
    - `wallet_address`: The Ethereum wallet address to perform forensic analysis on.
    - `max_depth`: The maximum depth for tracing transactions. Use -1 for full depth.
    ...
    **Authentication:** Requires an API key passed in the `X-API-Key` header.
    ...
    print(f"[+] Launching Wallet_ID_Monitor_Forensic.py as subprocess for {wallet_address}...", file=sys.stderr)
    ...
    try:
        ...
        if process.stderr:
            print(f"[Subprocess STDERR for {wallet_address}]\n{process.stderr}", file=sys.stderr)
        ...
        print(f"Error executing external script for {wallet_address}:\n{e.stderr}", file=sys.stderr)
        raise HTTPException(
            status_code=500,
            detail=f"Error during forensic analysis in external script for {wallet_address}: {e.stderr.strip()}"
        )
    except json.JSONDecodeError as e:
        print(f"Error parsing JSON from external script for {wallet_address}: {e}\nSTDOUT: {process.stdout}\nSTDERR: {process.stderr}", file=sys.stderr)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to parse JSON output from external script for {wallet_address}. Raw output might be invalid: {e}"
        )
    except Exception as e:
        print(f"An unexpected error occurred in the API for {wallet_address}: {e}", file=sys.stderr)
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred: {e}")

if __name__ == '__main__':
    import uvicorn
    # To run this: uvicorn forensic_api:app --reload --port 8003
    # Ensure you replace 'forensic_api' with the actual filename if different.
